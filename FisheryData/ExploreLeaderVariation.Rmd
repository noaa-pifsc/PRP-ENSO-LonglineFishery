...
```{r}
# Load data
ObservedSets_all <- readRDS(here("FisheryData", "ObservedDeepSetDetails.rds"))
```

```{r}
# Limit to geographic area of interest
ObservedSets <- ObservedSets_all |>
  filter(HAUL_BEGIN_LAT >= 10 & HAUL_BEGIN_LAT <= 40 & HAUL_BEGIN_LON >= -180 & HAUL_BEGIN_LON <= -130)
```
...
for (yr in seq(1995, 2024, 1)) {
  for (leader in seq(1, 4, 1)) {  
    ...
    if (length(leader_match) > 0) {
      ...
      leader_dist[yr-1994,leader] <- as.numeric(n[leader_match,2] / dim(yr_hold)[1])
    }
  }
}
...
plot(1:360, leader_dist_mon[,4], type = "l", col = "#a5aaaf", xlim = c(1,360), ylim = c(0,1),
     ylab = " ", xlab = " ", xaxt = "n", yaxt = "n")
par(new = TRUE)
plot(c(68, 68), c(0, 1), type = "l", col = "#cbcfd1", xlim = c(1,360), ylim = c(0,1),
...
plot(301:360, leader_dist_mon[301:360,2], type = "l", col = "#db6015", xlim = c(301,360), ylim = c(0,1),
     ylab = " ", xlab = " ", las = 1, xaxt = "n")
par(new = TRUE)
plot(c(317.5, 317.5), c(0, 1), type = "l", col = "#A5AAAF", xlim = c(301,360), ylim = c(0,1),
     ylab = " ", xlab = " ", las = 1, xaxt = "n")
