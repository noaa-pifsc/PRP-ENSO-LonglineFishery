...
```{r}
# This code combines the sql queries above (except the catch) with the joining and filtering from the ObserverCombine.Rmd script
ObservedDeepSetDetails <- sqlQuery(ch, paste(
  "SELECT",
    "cf.T_TRIP_NUM,", 
    "cf.S_SET_NUM,", 
    "cf.HKS_PER_FLT,", 
    ...
    "en.HAUL_BEGIN_LAT,", 
    "en.HAUL_BEGIN_LON,", 
    "en.HAUL_BEGIN_DATETIME,",
    "tr.PERMIT_NUM,", 
    "tr.EXPR_TRIP_TYPE_ID",
  "FROM",
    tname_obs_gearcfg, "cf",
    "FULL JOIN", tname_obs_setenvir, "en ON (cf.T_TRIP_NUM=en.T_TRIP_NUM AND cf.S_SET_NUM=en.SET_NUM)",
    "FULL JOIN", tname_obs_trips, "tr ON (cf.T_TRIP_NUM=tr.TRIP_NUM)",
  "WHERE",
    "tr.TRIP_NUM LIKE 'LL%' AND", 
    "cf.HKS_PER_FLT > 14 AND",
    "tr.EXPR_TRIP_TYPE_ID IS NULL")
)
